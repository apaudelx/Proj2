...
def train_lstm(model, train_loader, val_loader, epochs=30, lr=0.001, clip=1.0, device='cuda', patience=3):
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        val_loss, val_ppl = evaluate_lstm(model, val_loader, criterion, device)
        val_losses.append(val_loss)

        print(f"Epoch {epoch+1}/{epochs}")
        print(f"Train Loss: {avg_train_loss:.4f}, Train PPL: {train_ppl:.4f}")
...
def main():
    ...
    criterion = nn.CrossEntropyLoss(ignore_index=0)
    val_loss, val_ppl = evaluate(lstm_model, test_loader, criterion, device=device)
    print(f"\nFinal Validation Loss: {val_loss:.4f}")
    print(f"Final Validation Perplexity: {val_ppl:.4f}")

    avg_bleu, _ = calculate_bleu(lstm_model, sp, test_samples, device=device)
    print(f"Average BLEU score: {avg_bleu:.4f}")
    torch.save(lstm_model.state_dict(), "lstm_model.pt")

    print("\nSample text generation from LSTM:")
    prompt1 = "Which do you prefer? Dogs or cats?"
    generated1 = lstm_model.prompt(sp, prompt1, max_seq_length=100)
    print(f"\nPrompt: {prompt1}")
